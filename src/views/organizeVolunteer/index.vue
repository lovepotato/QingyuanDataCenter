  ...
  <div class="volunteer-container">
    <div class="count-list-box">
      <div
        :key="index"
        class="count-item"
        v-for="(countItem, index) in voluteerData.countList"
      >
    ...
    <div class="community-container">
      <div class="community-list">
        <vue-scroll :ops="ops">
          <div
            :class="{'active-community' : communityItem.id === activeCommunityId}"
            :key="index"
            @click="activeCommunityChange(communityItem.id)"
            class="list-item"
            v-for="(communityItem, index) in voluteerData.communityList"
          >
            <div
              :title="communityItem.title"
              class="community-title community-item text-overflow-class"
            >{{ communityItem.title }}</div>
      ...
      <div class="org-list">
        <div
          :key="index"
          class="list-item"
          v-for="(orgItem, index) in orgDetail.organizationList"
        >
          <div class="list-base-info">
            <div class="list-img">
              <el-image
                :src="orgItem.img | formatImageSrc"
                fit="fit"
            ...
            <div class="list-info-content">
              <div class="top-info">
                <div class="info-warper">
                  <div
                    :class="[orgInfoArray[0].iconName]"
                    class="info-item img-icon"
                  ></div>
          ...
          <div class="count-list">
            <div
              :key="sindex + 'count'"
              class="count-item"
              v-for="(item, sindex) in orgItem.countList"
            >
      ...
      <div class="activity-content">
        <div class="activity-chart">
          ...
          <div class="chart-item">
            <pie-chart
              :data="orgDetail.activityCategoryList"
              :option="activityCategoryOption"
            ></pie-chart>
          </div>
        </div>
          ...
          <div class="activity-items">
            <vue-scroll :ops="ops">
              <div
                :key="index"
                class="activity-item"
                v-for="(activityItem, index) in orgDetail.activityLastList"
              >
                <div
                  :class="{'hot-activity' : index < 3}"
                  class="item-name"
                >{{ activityItem.name }}</div>
...
export default {
  ...
  data() {
    return {
      ...
      orgDetail: {},
      orgInfoArray: [
        { iconName: 'lianxiren', title: '联系人', key: 'contacts' },
        { iconName: 'dianhua', title: '联系电话', key: 'mobile' },
        { iconName: 'dizhi', title: '联系地址', key: 'address' }
      ],
  ...
  methods: {
    getVoluteerData() {
      this.http
        .post(`/commandcentre/publicwelfareorganization/community`)
        .then(({ data, code }) => {
          if (code === 0) {
            this.voluteerData = data
            const activeId =
              data.communityList.length > 0 ? data.communityList[0].id : ''
    ...
    getOrgList(id) {
      this.http
        .post(`/commandcentre/publicwelfareorganization/community/org`, { id })
        .then(({ data, code }) => {
          if (code === 0) {
            this.orgDetail = data
          }
