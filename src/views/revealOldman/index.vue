  ...
  <div class="revealOldman-container">
    ...
    >
        ...
        <div class="safe-record-list">
          ...
          <div class="list-container">
            ...
            >
              <swiper-slide
                :key="index"
                v-for="(boardItem, index) in oldmanLastSafeData"
              >
      ...
      <div class="center-info">
        ...
        <div class="oldman-count-list">
          <div
            :key="index"
            class="count-item"
            v-for="(item, index) in oldmanLastCountListPre"
          >
        ...
        <div class="care-info-list">
          <div
            :key="index"
            class="care-count-item"
            v-for="(item, index) in oldmanLastCountListLast"
          >
            <div class="item-value">{{ item.value }}</div>
            <div class="item-title">{{ item.name }}</div>
          </div>
      ...
      <div class="all-oldman-list">
        <oldman-info-box
          :key="index"
          :oldman-detail="item"
          v-for="(item, index) in mapOldmanList"
        ></oldman-info-box>
      </div>
    </div>
    <!-- </transition> -->
    <div class="box-view-warper">
      <vue-scroll>
        <transition name="moveR">
          <div
            class="revealOldman-box-view"
            v-show="isDisplayAll"
          >
            <oldman-info-box
              :key="index"
              :oldman-detail="item"
              v-for="(item, index) in oldmanLastData"
            ></oldman-info-box>
          </div>
        </transition>
      </vue-scroll>
    ...
    <div class="fixed-banner">
      ...
      <div
        :key="index"
        class="banner-item"
        v-for="(item, index) in oldmanLastCount.todayCountList"
      >
    ...
    <div
      :class="{'diplay-all' : isDisplayAll,'disableClass':oldmanLastData.length==0}"
      @click="toggleDisplayAllBox"
...
export default {
  ...
  methods: {
    ...
    getOldmanData() {
      this.http
        .post(`/commandcentre/oldmanlast/list`, {
          limit: 10000
        })
        .then(({ data, code }) => {
          if (code === 0) {
    ...
    getOldmanLastCount() {
      this.http
        .post(`/commandcentre/oldmanlast/count`, {
          limit: 10000
        })
        .then(({ data, code }) => {
          if (code === 0) {
    ...
    getOldmanLastSafe() {
      this.http
        .post(`/commandcentre/oldmanlast/safe`)
        .then(({ data, code }) => {
          if (code === 0) {
