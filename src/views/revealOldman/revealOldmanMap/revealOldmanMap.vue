<template>
  <div class="map-content">
    ...
    ></div>
    <div
      :class="['org-id-'+i,oldmanCountList[i-1] > 0 ? '' : 'no-oldman-community']"
      :key="i"
    ...
    >
      <div
        class="reveal-oldman-tag"
        v-if="oldmanCountList && oldmanCountList[i-1]> 0"
      >
  ...
  methods: {
    ...
    getCommunityOldmanCountList() {
      ...
      this.communityIdList.forEach((item) => {
        const communityOldList = _.communityMemberLastCountList.find(
          (dataItem) => dataItem.index === item
        )
